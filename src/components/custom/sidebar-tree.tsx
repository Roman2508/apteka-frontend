...
const NAV_ITEMS = [
  {
    ...
    items: [
      ...
      },
      {
        ...
        links: [
          { path: "", label: "Причини відмов від покупок", isDisabled: false },
        ],
      },
      {
        title: "Інше",
...
export function SidebarTree() {
  ...
  return (
    <aside className="bg-[#FBED9E] border-r border-[#C0C0C0] flex flex-col">
      {/* <aside className="w-56 bg-[#FBED9E] border-r border-[#C0C0C0] flex flex-col"> */}
      {/* Заголовок */}
      <div className="w-22 h-[35px] bg-[#FFF1A1] border-b border-[#C0C0C0] flex items-center px-2">
...
function TreeItem({
  ...
  return (
    <div className="">
      ...
      {open && (
        <div className="absolute z-[9] left-[100%] top-0 h-[calc(100vh-35px)] w-[calc(100vw-100%)] bg-white border border-[#C0C0C0] shadow-lg p-6">
          <div className="flex flex-col flex-wrap max-h-[80%] inline-flex gap-x-6">
            {NAV_ITEMS.find((item) => item.id === id)?.items.map((section) => (
              <div key={section.title} className="p-4">
                <h3 className="text-green-500 mb-2 text-base">
                  {section.title}
                </h3>
                <ul>
                  {section.links.map((link) => (
                    <li
                      key={link.label}
                      className="mb-1 text-[13px] hover:underline"
                    >
                      <a href="">{link.label}</a>
                    </li>
                  ))}
