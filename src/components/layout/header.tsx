...
export function Header() {
  ...
  const onLogout = () => {
    if (!window.confirm("Ви дійсно хочете закрити зміну?")) return
    logout.mutate()
  }

  return (
    <Tabs value={activeTab} onValueChange={setActiveTab} className="flex flex-col">
      <div className="border-b border-neutral-800 bg-neutral-300 h-[34px]">
        <div className="flex items-center justify-between pr-4 h-[34px]">
          <TabsList className="inline-flex h-auto p-0 bg-transparent border-none rounded-none min-w-max">
            ...
            <div className="px-3 py-1 text-sm font-medium flex items-center">{shiftInfo}</div>
